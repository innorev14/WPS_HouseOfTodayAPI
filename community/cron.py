...
def community_todaystory():
    ...
    myList = []
    num_2nd = random.randrange(1, 19)

    for j in range(1, 6):
        if j == 4:
            continue
        while num_2nd in myList:
            num_2nd = random.randrange(1, 19)
        myList.append(num_2nd)
        HotStoryNumber(id=j, product_rnd_number=num_2nd).save()


    # for j in range(1, 4):
    #     num_2nd = random.randrange(1, 18)
    #     HotStoryNumber(id=j, product_rnd_number=num_2nd).save()

    # HotStoryNumber(id=5, product_rnd_number=random.randrange(1, 18)).save()
